...
import {AzureCognitiveService, AzureCognitiveState} from './azure-cognitive.service';
...
export class ChanDaLiarService {
  state$ = combineLatest([
    ...
    map(([openAi, azureCognitive, device]) => this.mapState(openAi, azureCognitive, device)),
    shareReplay(),
  );
  ...
  mapState(openAi: OpenAIState, azureCognitive: AzureCognitiveState, device: DeviceState): ChanDaLiarState {
    return {
